...
util.mixin(Resource.prototype, /** @lends Resource# */{
    ...
    renderHtml: function(req, res, next, resource, route, router) {
        var context = req.context,
            template = context.xrest.template,
            prefix = context.xrest.prefix;
    ...
    renderJson: function(req, res, next) {
        var data = req.context.locals || {};
        res.send(JSON.stringify(data) + '\n');
    },

    renderJs: function(req, res, next) {
        var fn = req.param('???'); // xxx
        res.send('function '+fn+'() {'+
            JSON.stringify(req.context.locals || {}) // ???
        +'}');
    },

    renderTxt: function(req, res, next) {
        var data = req.context.locals || {};
        if (data.hasOwnProperty('toString')) {
            data = data.toString();
        } else {
            data = JSON.stringify(req.context.locals);
        }
        res.send(data);
    },
